...
for k in range(N_it):
    for i in range(1,N):
        for j in range(1,N):
           matrizU[i][j] = w*(((j*h)**2 + 1.)/(2.* np.sin(i*h) + (2.*i*h) * np.exp(-j*h)*((j*h)**2 + 1.))) * ( (np.sin(i*h)/((j*h)**2 + 1.))*(matrizU[i+1][j] + matrizU[i-1][j]) + i*h*np.exp(-j*h)*(matrizU[i][j+1] + matrizU[i][j-1]) ) + ( 1. - w ) * matrizU[i][j]
